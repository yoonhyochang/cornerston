...
export default async function createImageIdsAndCacheMetaData({
  ...
  const tempSopInstanceUID = instances[0][SOP_INSTANCE_UID].Value[0];
  // get InstanceMetadata
  const instanceMetadata = await client.retrieveInstanceMetadata({
    studyInstanceUID: StudyInstanceUID,
    seriesInstanceUID: SeriesInstanceUID,
    sopInstanceUID: tempSopInstanceUID,
  });

  const instance = instanceMetadata[0];
  const instanceInfo = {
    patientId: instance[PATIENT_ID]?.Value[0],
    patientName: instance[PATIENT_NAME]?.Value && instance[PATIENT_NAME]?.Value[0]?.Alphabetic || '-',
    patientSex: instance[PATIENT_SEX]?.Value[0] || '-',
    patientAge: instance[PATIENT_AGE]?.Value[0] || '-',
    patientBirth: instance[PATIENT_BIRTH]?.Value[0] || '-',
    accessNumber: instance[ACCESSION_NUMBER]?.Value[0],
    seriesNumber: instance[SERIES_NUMBER]?.Value[0],
  };
